...
from sam3.train.data.sam3_image_dataset import Datapoint, Image, Object, InferenceMetadata
from sam3.model.box_ops import box_xywh_to_xyxy
from maple_layers import FindQueryLoaded, apply_maple_to_model, count_parameters, save_maple_weights
...
def create_coco_gt_from_dataset(dataset, image_ids=None, mask_resolution=288):
    ...
    coco_gt = {
        'info': {
            'description': 'SAM3 MaPLe Validation Dataset',
            'version': '1.0',
            'year': 2026
        },
        'images': [],
...
class SAM3TrainerNative:
    def __init__(self, config_path):
        ...
        self.model = build_sam3_image_model(
            ...
            compile=False,
            load_from_HF=self.config["model"]["load_from_HF"], # Tries to download from HF if checkpoint_path is None
            bpe_path="sam3/assets/bpe_simple_vocab_16e6.txt.gz",
            eval_mode=False,
        )

        # Apply MaPLe
        print("Applying MaPLe...")
        self.model = apply_maple_to_model(self.model, self.device.type, self.config["MaPLe"])
        stats = count_parameters(self.model)
        print(f"Trainable params: {stats['trainable_parameters']:,} ({stats['trainable_percentage']:.5f}%)")
    ...
    def train(self):
        ...
        print(f"Prefix prompt words: {self.config['MaPLe']['prefix']['prompt']}. Tuning: {self.config['MaPLe']['prefix']['tuning']}")
        print(f"Suffix prompt words: {self.config['MaPLe']['suffix']['prompt']}. Tuning: {self.config['MaPLe']['suffix']['tuning']}")
        ...
        for epoch in range(epochs):
            ...
            if has_validation and val_loader is not None:
                ...
                save_maple_weights(self.model, str(out_dir / "last_maple_weights.pt"))

                if avg_val_loss < best_val_loss:
                    best_val_loss = avg_val_loss
                    save_maple_weights(self.model, str(out_dir / "best_maple_weights.pt"))
        ...
        if has_validation:
            ...
            print(f"  - best_maple_weights.pt (best validation loss)")
            print(f"  - last_maple_weights.pt (last epoch)")
            print(f"{'='*80}")
        else:
            # If no validation, copy last to best
            import shutil
            last_path = out_dir / "last_maple_weights.pt"
            best_path = out_dir / "best_maple_weights.pt"
            if last_path.exists():
            ...
            print(f"{'='*80}")
            print(f"\nModels saved to {out_dir}:")
            print(f"  - best_maple_weights.pt (copy of last epoch)")
            print(f"  - last_maple_weights.pt (last epoch)")
            print(f"\nℹ️  No validation data - consider adding data/valid/ for better model selection")
            print(f"{'='*80}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train SAM3 with Prompt Learning")
    parser.add_argument(
        "--config",
        type=str,
