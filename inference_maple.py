#!/usr/bin/env python3
"""
Inference script for SAM3 with PLe weights.

This script loads the SAM3 model, applies PLe configuration,
...
from sam3.train.data.sam3_image_dataset import Datapoint, Image, InferenceMetadata
from sam3.train.data.collator import collate_fn_api
from maple_layers import FindQueryLoaded, apply_maple_to_model, load_maple_weights

from torchvision.transforms import v2


class SAM3PLeInference:
    def __init__(self, config_path, weights_path):
        """
        Initialize SAM3 PLe inference.

        Args:
            config_path: Path to YAML config file used for training
            weights_path: Path to saved PLe weights (.pt file)
        """
        ...
        self.model = build_sam3_image_model(
            ...
            compile=False,
            load_from_HF=self.config["model"]["load_from_HF"],
            bpe_path="sam3/assets/bpe_simple_vocab_16e6.txt.gz",
            eval_mode=True  # Set to eval mode for inference
        )

        # Apply PLe (with same config as training)
        print("Applying PLe...")
        self.model = apply_maple_to_model(self.model, self.device.type, self.config["MaPLe"])

        # Load PLe weights
        print(f"Loading PLe weights from {weights_path}...")
        self.maple_state_dict = load_maple_weights(weights_path)
        self.maple_state_dict["maple_inputs_embeds"].to(self.device)
    ...
    def predict(self, image_path, text_prompt=None):
        ...
        self.maple_state_dict["text"] = text_prompt
        print()
        datapoint.find_queries[0].query_text = self.maple_state_dict
...
def main():
    parser = argparse.ArgumentParser(description="SAM3 PLe Inference")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to trained PLe weights (.pt file). If not specified, uses best_maple_weights.pt from config's output_dir"
    )
    parser.add_argument(
        "--image",
        type=str,
    ...
    if args.weights is None:
        ...
        output_dir = config.get('output', {}).get('output_dir', 'outputs/sam3_maple_full')
        args.weights = os.path.join(output_dir, 'best_maple_weights.pt')
