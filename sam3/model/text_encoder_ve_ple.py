...
class VETextEncoder(nn.Module):
    def __init__(
        ...
        if self.config is not None:
            ...
            for i, values in enumerate(self.config["learning_prompts"]):
                prompts = self.tokenizer(values["prompt"], context_length=self.context_length)
                with torch.no_grad():
                    # embedding = self.encoder.token_embedding(prompts)[:, 1:len(prompts)+1].cuda()
                    embedding = self.encoder.token_embedding(prompts)
                    print(embedding)
                    print(embedding[:, 1:len(prompts)+1, :])
                    print()
                if values["tuning"]:
                    self.register_parameter(f'embedding_{i}', nn.Parameter(embedding))
                else:
                    self.register_buffer(f'embedding_{i}', embedding)
    ...
    def forward(
        ...
        if isinstance(text[0], str):
            ...
            inputs_embeds = torch.cat([getattr(self, f'embedding_{i}') for i in len(self.config["learning_token"])], dim=1)
